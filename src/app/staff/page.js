...
const StaffList = () => {
  ...
  const createData = (si, row, staffName, designation, mobile, email, address, salary, joiningDate, status) => ({
    si,
    staffName,
    designation,
    mobile,
    ...
    action: (
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <IconButton
          style={{ color: "#072eb0", padding: "4px" }}
          onClick={() => handleView(row)}
        >
          <VisibilityIcon />
        </IconButton>
        <IconButton
          style={{ color: "#6b6666", padding: "4px" }}
          onClick={() => handleEdit(row)}
        >
          <EditIcon />
        </IconButton>
        <IconButton
          style={{ color: "#e6130b", padding: "4px" }}
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch(`${Base_url}/staff/${deleteId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    <Layout>
      ...
      <Box sx={{ p: 3 }}>
        ...
        <CommonDialog
          ...
          onClose={handleClose}
          dialogTitle={
            openData ? "Create New Staff" : viewShow ? "View Staff" : editShow ? "Edit Staff" : "Delete Staff"
          }
          dialogContent={
            openData ? (
              ...
              <EditStaff editData={editData} handleUpdate={handleUpdate} handleClose={handleClose} />
            ) : deleteShow ? (
              <DeleteStaff handleDelete={handleDelete} isDeleting={isDeleting} handleClose={handleClose} />
            ) : null
          }
        />
      </Box>
